...
int Flatulan::col() const //COMPLETED
{
	return m_col;
}
...
void Flatulan::move()
{
	...
	int dir = randInt(0, NUMDIRS - 1);  // dir is now UP, DOWN, LEFT, or RIGHT
										//******ASK IF I DID THIS RIGHT ************
	switch (dir)
	{
	case UP:
		if (row() - 1 >= 1)
		{
			if (!(row() - 1 == m_city->player()->row() && col() == m_city->player()->col()))
			{
				m_row = row() - 1;
			}
			else
	...
	case LEFT:
		if (col() - 1 >= 1)
		{
			if (!(row() == m_city->player()->row() && col() - 1 == m_city->player()->col()))
			{
				m_col = col() - 1;
			}
			else
...
bool Flatulan::possiblyGetConverted()  // return true if converted //*******ASK IF I DID THIS RIGHT*******
{
...
int Player::row() const //COMPLETED
{
	return m_row;
}
...
int Player::age() const //COMPLETED
{
	return m_age;
}
...
void Player::getGassed() // ***DOES THIS MEAN IF THE PLAYER IS GASSED BY A FLATULAN???***
{
	m_health--;
}
...
{
	int numOfFlatulansAtSomeCoord = 0;
	for (int x = 0; x < m_nFlatulans; x++)
	{
		if (m_flatulans[x]->row() == r && m_flatulans[x]->col() == c)
		{
			numOfFlatulansAtSomeCoord++;
		}
	}
	return numOfFlatulansAtSomeCoord;
}
...
void City::display() const
{
	...
	// Indicate each Flatulan's position                                   //*********I DO NOT KNOW IF I DID THIS RIGHT!!!!!!!!!!*****
	int numOfFlatulans = 0;
	for (r = 0; r < rows(); r++)
	{
		for (c = 0; c < cols(); c++)
		{
			numOfFlatulans = nFlatulansAt(r + 1, c + 1);
			switch (numOfFlatulans)
			{
			case 0:
				break;
			case 1:
...
bool City::addFlatulan(int r, int c)
{
	...
	{
		m_flatulans[m_nFlatulans] = new Flatulan(this, r, c); // **************ASK IF I DID THIS RIGHT!!!!!!*********** //************ASK IF NEED TO DEFELETE A FLATULAN BUT I DO NOT THINK SO!!!!****
		m_nFlatulans++;                                       //****SEE IF THIS FUNCTION CAN BE RELATED TO THE CONVERSION FUNCTION!!!!!!!!**********
		return true;
	}
}
...
void City::preachToFlatulansAroundPlayer()
{
	...
	for (int x = 0; x < m_nFlatulans; x++) //*******************ASK IF I DID THIS RIGHT*******************
	{
		if (m_flatulans[x]->row() == player()->row() - 1 && m_flatulans[x]->col() == player()->col())
		{
			if (m_flatulans[x]->possiblyGetConverted() == true)
			{
				int i = x;
				delete m_flatulans[x];
				while (i < m_nFlatulans - 1)
				{
					m_flatulans[i] = m_flatulans[i + 1];	//shifts elements of array over
					i++;
				}  //******ASK IF BOTH ARE OKAY!!!!!!*******///
				   /*m_flatulans[x] = m_flatulans[m_nFlatulans - 1];*/
				m_flatulans[m_nFlatulans - 1] = NULL;
				m_nFlatulans--;
				x--;
			}
		}
		else if (m_flatulans[x]->row() == player()->row() + 1 && m_flatulans[x]->col() == player()->col())
		{
			if (m_flatulans[x]->possiblyGetConverted() == true)
			{
				int i = x;
		...
		else if (m_flatulans[x]->row() == player()->row() && m_flatulans[x]->col() == player()->col() - 1)
		{
			if (m_flatulans[x]->possiblyGetConverted() == true)
			{
				int i = x;
		...
		else if (m_flatulans[x]->row() == player()->row() - 1 && m_flatulans[x]->col() == player()->col() - 1)
		{
			if (m_flatulans[x]->possiblyGetConverted() == true)
			{
				int i = x;
...
void City::moveFlatulans()
{
	...
	{
		m_flatulans[k]->move();
		if (m_flatulans[k]->row() == player()->row() - 1 && player()->col() == m_flatulans[k]->col())
		{
			player()->getGassed();
		}
		...
		if (m_flatulans[k]->row() == player()->row() && m_flatulans[k]->col() == player()->col() - 1)
		{
			player()->getGassed();
		}
		if (m_flatulans[k]->row() == player()->row() && m_flatulans[k]->col() == player()->col() + 1)
